...
import time
import re
# from aaDict_CIViC import aaCodes
...
if (not genes) and (not skipGenes):
    print("\nDid not find any genes in column \'{}\' of file {}".format(args.colName_gene, args.inputFile))
    outfile.close()
    sys.exit(0)
# When no genes were found because all existing were skipped (symbol containing dots '.'),
# write output identical to input file plus additional CIVIC columns being empty
elif (not genes) and skipGenes:
    print("\nCould not find any genes that allow query to CIVIC (all were skipped due to non-allowed gene symbols).")
...
## TODO: optimize batchSize value or set as parameter?
batchSize = 200
