...
# FIXME: sanity check potential issues due to skipping non c.HGVS variant annotations -> what would happen if all annotations for the line are n.HGVS? This would likely trigger errors, so we need to ensure that in these cases a tier4 is reported? what other alternative for the output table?
        ...
        for gene in genes_and_variants_to_write.keys():
            ...
            for variant_annotation in genes_and_variants_to_write[gene].keys():
                ...
                if tier_to_write == "tier_4":
                    ...
                    drug_support = match_mapping[gene][variant_annotation][tier_to_write]["drug_support"]
                    if write_support:
                ...
                for variant_id in all_civic_variants:
                    ...
                    # This is a dummy tag and not an actual variant record from CIVICdb, so skip checking in variant_mapping
                    ...
                    gene_scores.append(gene + ':' + civic_variant + ':' + str(variant_mapping[gene][variant_id]['civic_score']))
                    gene_var_types.append(gene + ':' + civic_variant + ':' + ','.join(variant_mapping[gene][variant_id]['types']))
                    for evidence_type in sorted_evidence_types:
                        if evidence_type in variant_mapping[gene][variant_id]["evidence_items"].keys():
                            if evidence_type not in result_mapping.keys():
                                result_mapping[evidence_type] = []
                            writeDrug = False
                            ...
                            if evidence_type == drug_evidence_type:
                                writeDrug=True
                            if has_ct:
                                check_keys(list(variant_mapping[gene][variant_id]["evidence_items"][evidence_type].keys()), "variant_mapping", sorted_cts, matches_all=True)
                                for ct in variant_mapping[gene][variant_id]["evidence_items"][evidence_type].keys():
                                    if write_ct:
                                        results = write_evidences(variant_mapping[gene][variant_id]["evidence_items"][evidence_type][ct], writeDrug=writeDrug, write_ct=ct, write_complete=write_complete)
                                    else:
                                        results = write_evidences(variant_mapping[gene][variant_id]["evidence_items"][evidence_type][ct], writeDrug=writeDrug, write_ct=None, write_complete=write_complete)
                                    for x in results:
                                        result_mapping[evidence_type].append(gene + ":" + civic_variant + ":" + x)
                            else:
                                check_keys_not(list(variant_mapping[gene][variant_id]["evidence_items"][evidence_type].keys()),"variant_mapping", sorted_cts)
                                if write_ct:
                                    raise ValueError("Option 'write_ct' cannot be selected when 'has_ct'=False!")
                                results = write_evidences(variant_mapping[gene][variant_id]["evidence_items"][evidence_type], writeDrug=writeDrug, write_ct=None, write_complete=write_complete)
                                for x in results:
                                    result_mapping[evidence_type].append(gene + ":" + civic_variant + ":" + x)
        ...
        drug_support = []
        drug_support = reprocess_drug_support_across_selected_variants(genes_and_variants_to_write, match_mapping, variant_mapping, supportDict, has_support=True)

        ## Write result to output
        outLine = write_output_line(tier_to_write, line.strip(), gene_scores, gene_var_types, drug_support, result_mapping, write_support)
...
if data_type=='SNV':
    (raw_mapping, varData, header) = read_in_snvs_multiple_genes(args.infile)
# Read in and process file of input CNV variants
if data_type=='CNV':
    (raw_mapping, varData, header) = read_in_cnvs_multiple_genes(args.infile)
...
# (variant_mapping,retrieved_genes,no_variants,all_variants) = query_civic_genes(genes, identifier_type="entrez_symbol", dataType=data_type)
...
(match_mapping, matchedIds, variant_mapping) = match_in_civic(varData, dataType=data_type, identifier_type="entrez_symbol", select_tier="highest", varMap=variant_mapping)
...
annot_match_mapping = process_drug_support(match_mapping, annot_mapping, supportDict)
