...
def filter_in(field, field_name, in_list, list_name, match_type="exact"):
    ...
    """
    (field, in_list) = check_string_filter_arguments(
        field, field_name, in_list, list_name)
    match = False
...
def filter_not_in(field, field_name, out_list, list_name, match_type="exact"):
    ...
    """
    (field, out_list) = check_string_filter_arguments(
        field, field_name, out_list, list_name)
    match = False
...
def filter_civic(var_map, gene_id_in=[], gene_id_not_in=[], min_variants=0, var_id_in=[], var_id_not_in=[], var_name_in=[], var_name_not_in=[], min_civic_score=0, var_type_in=[], var_type_not_in=[], min_evidence_items=0, evidence_type_in=[], evidence_type_not_in=[], disease_in=[], disease_not_in=[], drug_name_in=[], drug_name_not_in=[], evidence_dir_in=[], evidence_dir_not_in=[], evidence_clinsig_in=[], evidence_clinsig_not_in=[], evidence_level_in=[], evidence_level_not_in=[], evidence_status_in=[], evidence_status_not_in=[], source_status_in=[], source_status_not_in=[], var_origin_in=[], var_origin_not_in=[], source_type_in=[], source_type_not_in=[], min_evidence_rating=0, output_empty=False):
    ...
    var_map_entries_molecular_profile = [
        "civic_score", "n_evidence_items", "evidence_items"]
    ...
    for gene_id in var_map.keys():
        ...
        keep_gene = filter_in(gene_id_str, "gene_id",
                              gene_id_in, "gene_id_in", match_type="exact")
        if not keep_gene:
            continue
        remove_gene = filter_not_in(
            gene_id_str, "gene_id", gene_id_not_in, "gene_id_not_in", match_type="exact")
        if remove_gene:
            continue

        # Allow number of min_variants to be 0
        n_variants = len(var_map[gene_id].keys())
        keep_gene = filter_cutoff(
            n_variants, "n_variants", min_variants, "min_variants")
        if not keep_gene:
            continue
        ...
        for var_id in var_map[gene_id].keys():
            ...
            keep_var = filter_in(var_id_str, "var_id",
                                 var_id_in, "var_id_in", match_type="exact")
            if not keep_var:
                continue
            remove_var = filter_not_in(
                var_id_str, "var_id", var_id_not_in, "var_id_not_in", match_type="exact")
            if remove_var:
                continue

            # Check that the expected entries are found in the dictionary
            check_keys(list(var_map[gene_id][var_id].keys(
            ...
            keep_var = filter_in(
                variant, "variant", var_name_in, "var_name_in", match_type="partial")
            if not keep_var:
                continue
            remove_var = filter_not_in(
                variant, "variant", var_name_not_in, "var_name_not_in", match_type="partial")
            if remove_var:
                continue
            ...
            for var_type in variant_types:
                keep_type = filter_in(
                    var_type, "var_type", var_type_in, "var_type_in", match_type="partial")
                if keep_type:
                    n_keep += 1
                remove_type = filter_not_in(
                    var_type, "var_type", var_type_not_in, "var_type_not_in", match_type="partial")
                if remove_type:
                    n_remove += 1
            if (n_keep == 0):
                continue
            ...
            molecular_profile_ids = set(
                list(var_map[gene_id][var_id].keys())) ^ set(var_map_entries_variant)

            for molecular_profil_id in molecular_profile_ids:
                # Check that the expected entries are found in the dictionary
                check_keys(list(var_map[gene_id][var_id][molecular_profil_id].keys(
                )), "var_map", var_map_entries_molecular_profile, matches_all=False)

                # Allow number of evidence items to be 0
                n_evidence_items = var_map[gene_id][var_id][molecular_profil_id]["n_evidence_items"]
                keep_mp = filter_cutoff(
                    n_evidence_items, "n_evidence_items", min_evidence_items, "min_evidence_items")
                if not keep_mp:
                    continue
                ...
                keep_mp = filter_cutoff(
                    var_score, "var_score", min_civic_score, "min_civic_score")
                if not keep_mp:
                    continue

                n_evidence_items_after = 0
                evidence_types = list(
                    var_map[gene_id][var_id][molecular_profil_id]["evidence_items"].keys())
                ...
                for evidence_type in evidence_types:
                    keep_type = filter_in(
                        evidence_type, "evidence type", evidence_type_in, "evidence_type_in", match_type="exact")
                    if not keep_type:
                        continue
                    remove_type = filter_not_in(
                        evidence_type, "evidence type", evidence_type_not_in, "evidence_type_not_in", match_type="exact")
                    if remove_type:
                        continue
                ...
                for evidence_type in allowed_evidence_types:
                    ...
                    type_diseases = list(
                        var_map[gene_id][var_id][molecular_profil_id]["evidence_items"][evidence_type].keys())
                    ...
                    for type_disease in type_diseases:
                        keep_disease = filter_in(
                            type_disease, "disease type", disease_in, "disease_in", match_type="partial")
                        if not keep_disease:
                            continue
                        remove_disease = filter_not_in(
                            type_disease, "disease type", disease_not_in, "disease_not_in", match_type="partial")
                        if remove_disease:
                            continue
                    ...
                    for disease in allowed_diseases:
                        ...
                        disease_drugs = list(
                            var_map[gene_id][var_id][molecular_profil_id]["evidence_items"][evidence_type][disease].keys())
                        ...
                        if (evidence_type != "PREDICTIVE"):
                            allowed_drugs = [
                                x for x in disease_drugs]   # ["NULL"]
                        else:
                            # Iterate existing drugs and apply filters
                            for disease_drug in disease_drugs:
                                keep_drug = filter_in(
                                    disease_drug, "drug", drug_name_in, "drug_name_in", match_type="partial")
                                if not keep_drug:
                                    continue
                                remove_drug = filter_not_in(
                                    disease_drug, "drug", drug_name_not_in, "drug_name_not_in", match_type="partial")
                                if remove_drug:
                                    continue
                        ...
                        for drug in allowed_drugs:
                            ...
                            evidences = list(var_map[gene_id][var_id][molecular_profil_id]
                                             ["evidence_items"][evidence_type][disease][drug].keys())
                            allowed_evidences = []
                            for evidence in evidences:
                                ...
                                keep_dir = filter_in(
                                    direction, "evidence direction", evidence_dir_in, "evidence_dir_in", match_type="exact")
                                if not keep_dir:
                                    continue
                                remove_dir = filter_not_in(
                                    direction, "evidence direction", evidence_dir_not_in, "evidence_dir_not_in", match_type="exact")
                                if remove_dir:
                                    continue
                                # Check evidence clinical significance filters
                                clin_signf = evidence_list[1]
                                keep_clin = filter_in(
                                    clin_signf, "clinical significance", evidence_clinsig_in, "evidence_clinsig_in", match_type="exact")
                                if not keep_clin:
                                    continue
                                remove_clin = filter_not_in(
                                    clin_signf, "clinical significance", evidence_clinsig_not_in, "evidence_clinsig_not_in", match_type="exact")
                                if remove_clin:
                                    continue
                            ...
                            for this_evidence in allowed_evidences:
                                ...
                                evidence_levels = list(
                                    var_map[gene_id][var_id][molecular_profil_id]["evidence_items"][evidence_type][disease][drug][this_evidence].keys())
                                allowed_levels = []
                                for evidence_level in evidence_levels:
                                    keep_level = filter_in(
                                        evidence_level, "evidence level", evidence_level_in, "evidence_level_in",  match_type="exact")
                                    if not keep_level:
                                        continue
                                    remove_level = filter_not_in(
                                        evidence_level, "evidence level", evidence_level_not_in, "evidence_level_not_in", match_type="exact")
                                    if remove_level:
                                        continue
                                ...
                                for level in allowed_levels:
                                    ...
                                    all_evidence_items = var_map[gene_id][var_id][molecular_profil_id][
                                        "evidence_items"][evidence_type][disease][drug][this_evidence][level]
                                    for evidence_item in all_evidence_items:
                                        ...
                                        if (len(item_list) != 5):
                                            raise ValueError(
                                                "Unexpected format of evidence item '%s'! Please provide string as 'EVIDENCE_ID:EVIDENCE_STATUS:SOURCE_STATUS:VARIANT_ORIGIN:RATING'." % (evidence_item))
                                        tmp_id = item_list[0]
                                        ...
                                        if (len(tmp_id_list) != 2):
                                            raise ValueError(
                                                "Unexpected format of evidence id '%s'! Please provide string as 'PUBMED_[ID]' or 'ASCO_[ID]'." % (tmp_id))
                                        id_type, _ = tmp_id_list

                                        # Apply filters on evidence status
                                        keep_status = filter_in(
                                            evidence_status, "evidence status", evidence_status_in, "evidence_status_in", match_type="exact")
                                        if not keep_status:
                                            continue
                                        remove_status = filter_not_in(
                                            evidence_status, "evidence status", evidence_status_not_in, "evidence_status_not_in", match_type="exact")
                                        if remove_status:
                                            continue
                                        # Apply filters on source status
                                        keep_source = filter_in(
                                            source_status, "source status", source_status_in, "source_status_in", match_type="partial")
                                        if not keep_source:
                                            continue
                                        remove_source = filter_not_in(
                                            source_status, "source status", source_status_not_in, "source_status_not_in", match_type="partial")
                                        if remove_source:
                                            continue
                                        # Apply filters on variant origin
                                        keep_origin = filter_in(
                                            var_origin, "variant origin", var_origin_in, "var_origin_in", match_type="partial")
                                        if not keep_origin:
                                            continue
                                        remove_origin = filter_not_in(
                                            var_origin, "variant origin", var_origin_not_in, "var_origin_not_in", match_type="partial")
                                        if remove_origin:
                                            continue
                                        # Apply filters on source type
                                        keep_type = filter_in(
                                            id_type, "source id type", source_type_in, "source_type_in", match_type="exact")
                                        if not keep_type:
                                            continue
                                        remove_type = filter_not_in(
                                            id_type, "source id type", source_type_not_in, "source_type_not_in", match_type="exact")
                                        if remove_type:
                                            continue
                                        ...
                                        # When rating is available, apply filters on evidence rating
                                            ...
                                            keep_rating = filter_cutoff(
                                                rating, "rating", min_evidence_rating, "min_evidence_rating")
                                            if not keep_rating:
                                                continue
                                        ...
                                        clean_map[gene_id][var_id][molecular_profil_id]["evidence_items"][evidence_type][disease][drug][this_evidence][level].append(
                                            evidence_item)
