...
def cnv_is_exon_string(var_name):
    ...
    """
    # NOTE: uppercase is critical for the match!
    exon_strings = ["^EXON [0-9-]+ DELETION$",
                    "^[35\']+ EXON DELETION$", "^EXON [0-9-]+ SKIPPING MUTATION$"]
    ...
    if data_type == "SNV":
        ...
        for var_annot in var_annotations:
            if var_annot not in match_strings:
                ...
                if re.match(r'(P\.TER[0-9]+[A-Z]+)EXT', var_annot):
                    new_var_annot = re.match(
                        r'(P\.TER[0-9]+[A-Z]+)EXT', var_annot).groups()[0]
        ...
        for i, exon in enumerate(exon_annots):
            ...
            if len(impact_annots) != len(exon_annots):
                raise ValueError(
                    "Provided 'impact_annots' and 'exon_annots' are not of identical length. Please provide one variant impact annotation for each available exon annotation.")
...
def get_expression_strings(gene, logfc):
    ...
    if logfc > 0:
        ...
        raise ValueError(
            "Invalid logFC = '%s' for gene '%s'. Only differentially expressed genes are valid." % (logfc, gene))
...
def match_variants_in_civic(gene, variants, var_map, data_type, impacts=[], exons=[]):
    ...
    match = {"tier_1": [], "tier_1b": [],
             "tier_2": [], "tier_3": [], "tier_4": False}
    # Sanity check of provided arguments
    check_data_type(data_type)
    ...
    (input_strings, is_exact, is_true_exact) = input_match_strings(
        variants, data_type, impacts, exons)
    ...
    if gene in var_map.keys():
        for var_id in var_map[gene].keys():
            ...
            civic_strings = civic_match_strings(
                variant_name, hgvs_expressions, data_type)
...
def match_expression_in_civic(gene, expression_strings, var_map):
    ...
    # NOTE: uppercase is critical for the match!
    gene = gene.upper()
    expression_strings = uppercase_list(
    ...
    if gene in var_map.keys():
        for var_id in var_map[gene].keys():
            ...
            civic_strings = civic_match_strings(
                variant_name, hgvs_expressions=[], data_type="EXPR")
...
def add_match(match_map, gene, variant, match):
    ...
    # Sanity check of provided arguments
    check_arguments([match_map, gene, variant, match], [
                    "match_map", "gene", "variant", "match"])
        ...
        for variant in var_data[gene].keys():
            ...
            if data_type == "SNV":
                ...
                if (len(variant_list) < 4):
                    raise ValueError(
                        "Must provide at least 4 fields to describe a SNV variant (even if some can be empty): 'dna|prot|impact|exon|..|'")
                ...
                c_variants_list = parse_input(
                    c_variants, "c_variants", is_required=True)
                # Field must exist but can contain empty values
                p_variants_list = parse_input(
                    p_variants, "p_variants", is_required=False)
                ...
                impact_list = parse_input(
                    impacts, "impacts", is_required=False)
                # Field is optional and can contain empty values
                ...
                if not variants:
                    raise ValueError(
                        "At least one non-empty variant annotation (either 'c.' or 'p.') must be provided per variant!")
            ...
            if data_type == "CNV":
                ...
                variants = parse_input(
                    cnv_variants, "cnv_variants", is_required=True)
            ...
            if (data_type == "SNV") or (data_type == "CNV"):
                match = match_variants_in_civic(
                    gene, variants, var_map, data_type, impacts=impact_list, exons=exon_list)
...
def classify_diseases(disease_list, disease_name_not_in, disease_name_in, alt_disease_names):
    ...
    disease_name_not_in = uppercase_list(
        disease_name_not_in, "disease_name_not_in")
...
def add_ct(diseases, ct, gene, variant, molecular_profile, evidence_type, new_map, var_map, is_annot=False):
    ...
    if ct not in sorted_cts:
        raise ValueError(
            "Provided ct '%s' is not valid! Please provide one of: %s" % (ct, sorted_cts))
    ...
    check_dict_entry(new_map[gene][variant][molecular_profile],
                     "new_map", "evidence_items", "evidence_items")
    check_dict_entry(new_map[gene][variant][molecular_profile]
                     ["evidence_items"], "new_map", evidence_type, "evidence_type")
    check_dict_entry(var_map, "var_map", gene, "gene")
    check_dict_entry(var_map[gene], "var_map", variant, "variant")
    check_dict_entry(var_map[gene][variant][molecular_profile],
                     "var_map", "evidence_items", "evidence_items")
    check_dict_entry(var_map[gene][variant][molecular_profile]
                     ["evidence_items"], "var_map", evidence_type, "evidence_type")

    # Check whether provided var_map is annotated with disease specificity info (ct/gt/nct) or not
    if is_annot:
        check_keys(list(var_map[gene][variant][molecular_profile]["evidence_items"]
                   [evidence_type].keys()), "var_map", sorted_cts, matches_all=True)
        # If a non-empty list of diseases was provided, check they exist in var_map
        if diseases:
            diseases = uppercase_list(diseases, "diseases")
            check_keys(list(var_map[gene][variant][molecular_profile]["evidence_items"]
                       [evidence_type][ct].keys()), "var_map", diseases, matches_all=False)
    else:
        check_keys_not(list(var_map[gene][variant][molecular_profile]
                       ["evidence_items"][evidence_type].keys()), "var_map", sorted_cts)
        # If a non-empty list of diseases was provided, check they exist in var_map
        if diseases:
            diseases = uppercase_list(diseases, "diseases")
            check_keys(list(var_map[gene][variant][molecular_profile]["evidence_items"]
                       [evidence_type].keys()), "var_map", diseases, matches_all=False)
                ...
                for evidence in var_map[gene][variant][molecular_profile]["evidence_items"][evidence_type][ct][disease][drug].keys():
                    ...
                    for evidence_level in var_map[gene][variant][molecular_profile]["evidence_items"][evidence_type][ct][disease][drug][evidence].keys():
                        ...
                        for thisString in var_map[gene][variant][molecular_profile]["evidence_items"][evidence_type][ct][disease][drug][evidence][evidence_level]:
                            new_map[gene][variant][molecular_profile]["evidence_items"][evidence_type][ct][disease][drug][evidence][evidence_level].append(
                                thisString)
        else:
...
def annotate_ct(var_map, disease_name_not_in, disease_name_in, alt_disease_names):
    ...
    for gene in var_map.keys():
        ...
        for variant in var_map[gene].keys():
            ...
            check_keys(list(var_map[gene][variant].keys(
            )), "var_map", var_map_entries_variant, matches_all=False)
            new_map[gene][variant]["name"] = var_map[gene][variant]["name"]
            new_map[gene][variant]["hgvs"] = [
                a for a in var_map[gene][variant]["hgvs"]]
            new_map[gene][variant]["types"] = [
                b for b in var_map[gene][variant]["types"]]

            molecular_profile_ids = set(
                list(var_map[gene][variant].keys())) ^ set(var_map_entries_variant)
            ...
            for molecular_profile_id in molecular_profile_ids:
                ...
                for evidence_type in var_map[gene][variant][molecular_profile_id]["evidence_items"].keys():
                    ...
                    all_diseases = list(
                        var_map[gene][variant][molecular_profile_id]["evidence_items"][evidence_type].keys())
                    ...
                    (ct_disease_list, gt_disease_list, nct_disease_list) = classify_diseases(
                        all_diseases, disease_name_not_in, disease_name_in, alt_disease_names)
                    # Add new layer of classification within the evidence types: specificity of the disease (ct, gt, nct)
                    for ct in sorted_cts:
                        if ct == "ct":
                            new_map = add_ct(ct_disease_list, ct, gene, variant, molecular_profile_id,
                                             evidence_type, new_map, var_map, is_annot=False)
                        if ct == "gt":
                            new_map = add_ct(gt_disease_list, ct, gene, variant, molecular_profile_id,
                                             evidence_type, new_map, var_map, is_annot=False)
                        if ct == "nct":
                            new_map = add_ct(nct_disease_list, ct, gene, variant, molecular_profile_id,
                                             evidence_type, new_map, var_map, is_annot=False)
    ...
    if isinstance(select_ct, str) and (select_ct == "all"):
        ...
        for gene in var_map.keys():
            for variant in var_map[gene].keys():
                ...
                for molecular_profile_id in molecular_profile_ids:
                    for evidence_type in var_map[gene][variant][molecular_profile_id]["evidence_items"].keys():
                        check_keys(list(var_map[gene][variant][molecular_profile_id]["evidence_items"][evidence_type].keys(
            ...
            for molecular_profile_id in molecular_profile_ids:
                ...
                for evidence_type in var_map[gene][variant][molecular_profile_id]["evidence_items"].keys():
                    ...
                    for ct in sorted_cts:
                        ...
                        ct_list = list(
                            var_map[gene][variant][molecular_profile_id]["evidence_items"][evidence_type][ct].keys())
                        ...
                        if isinstance(select_ct, str) and (select_ct == "highest"):
                            ...
                            if ct_list and (not skip):
                                new_map = add_ct(
                                    ct_list, ct, gene, variant, molecular_profile_id, evidence_type, new_map, var_map, is_annot=True)
            ...
            for tier in match_map[gene][variant].keys():
                ...
                if tier != "tier_4":
                    for var_id in match_map[gene][variant][tier]:
                        ...
                        check_dict_entry(var_map, "var_map", gene, "gene")
                        check_dict_entry(
                            var_map[gene], "var_map", var_id, "variant")

                        molecular_profile_ids = set(
                            list(var_map[gene][var_id].keys())) ^ set(var_map_entries_variant)
                        for molecular_profile_id in molecular_profile_ids:
                            check_dict_entry(
                                var_map[gene][var_id][molecular_profile_id], "var_map", "evidence_items", "evidence_items")
                            if evidence_type in var_map[gene][var_id][molecular_profile_id]["evidence_items"].keys():
                                ...
                                check_keys(list(var_map[gene][var_id][molecular_profile_id]["evidence_items"][evidence_type].keys(
                                )), "var_map", sorted_cts, matches_all=True)
                                    ...
                                    for disease in var_map[gene][var_id][molecular_profile_id]["evidence_items"][evidence_type][ct].keys():
                                        for drug in var_map[gene][var_id][molecular_profile_id]["evidence_items"][evidence_type][ct][disease].keys():
                                            ...
                                            if gene not in drug_target[drug].keys():
                                                ...
                                                drug_target[drug][gene].append(
                                                    var_map[gene][var_id]["name"])
                                                drug_target[drug][gene].append(
                                                    tier)
                                                drug_target[drug][gene].append(
                                                    evidence_type)
                                                ...
                                                drug_target[drug][gene].append(
                                                    disease)
                                                drug_target[drug][gene].append(
                                                    var_map[gene][var_id][molecular_profile_id]["evidence_items"][evidence_type][ct][disease][drug])
                                            ...
                                            for evidence in var_map[gene][var_id][molecular_profile_id]["evidence_items"][evidence_type][ct][disease][drug].keys():
                                                ...
                                                if ("NULL" in direction) or ("N/A" in direction) or ("NULL" in clin_signf) or ("N/A" in clin_signf):
                                                    ...
                                                    if direction not in support_dict.keys():
                                                        raise ValueError(
                                                            "Could not find evidence direction '%s' in provided 'support_dict'!" % (direction))
                                                    if clin_signf not in support_dict[direction].keys():
                                                        raise ValueError(
                                                            "Could not find clinical significance '%s' in provided 'support_dict'!" % (clin_signf))
                    ...
                    for this_drug in drug_map.keys():
                        for this_ct in drug_map[this_drug].keys():
                            ...
                            count_pos = drug_map[this_drug][this_ct].count(
                                "POSITIVE")
                            count_neg = drug_map[this_drug][this_ct].count(
                                "NEGATIVE")
                            count_unk = drug_map[this_drug][this_ct].count(
                                "UNKNOWN_BLANK")
                            count_dns = drug_map[this_drug][this_ct].count(
                                "UNKNOWN_DNS")
                            ...
                            if (count_pos == 0) and (count_neg == 0) and (count_total_unk == 0):
                                raise ValueError(
                                    "Encountered unexpected support case for gene '%s'." % (gene))
                            ...
                            new_map[gene][variant][tier]["drug_support"].append(
                                drug_support)
...
def reprocess_drug_support_across_selected_variants(input_data, match_map, var_map, support_dict, has_support=True):
    ...
    for gene in input_data.keys():
        if gene not in match_map.keys():
            raise ValueError(
                "Input gene '%s' from 'input_data' could not be found in provided 'match_map'!" % (gene))
        for variant in input_data[gene].keys():
            if variant not in match_map[gene].keys():
                raise ValueError(
                    "Input variant '%s' (gene '%s') from 'input_data' could not be found in provided 'match_map'!" % (variant, gene))
    ...
    for this_drug in drug_map.keys():
        for this_ct in drug_map[this_drug].keys():
            ...
            if (count_pos == 0) and (count_neg == 0) and (count_total_unk == 0):
                raise ValueError(
                    "Encountered unexpected support case when aggregating evidences across provided 'input_data'!")
            ...
            # Format: DRUG:CT:SUPPORT:#pos|#neg|#unk|#dns
            drug_support = this_drug + ":" + this_ct.upper() + ":" + temp_support + ":" + \
                str(count_pos) + "|" + str(count_neg) + "|" + \
                str(count_unk) + "|" + str(count_dns)
