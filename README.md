...
* **CIViC_Score**: semi-colon separated list of CIViC variant record(s) matched for the given input variant, along with their corresponding molecular profile id(s) and CIViC Actionability Scores. (ex: _<Variant_Name>:<molecular_profile_id>:<CIViC_score>_)
* **CIViC_VariantType**: semi-colon separated list of variant types reported in CIViC for the matched variant record(s).
* **CIViC_Drug_Support**: semi-colon separated list of consensus drug response predictions generated by CIViCutils based on the available predictive CIViC evidence matched for the input variant. Optional column, only reported when `write_support = True`. CIViC_PREDICTIVE column contains the evidence used for the consensus drug response predictions. 
